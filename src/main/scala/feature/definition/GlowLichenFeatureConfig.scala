...
case class GlowLichenFeatureConfig(searchRange: Int, canPlaceOnFloor: Boolean, canPlaceOnCeiling: Boolean, canPlaceOnWall: Boolean,
                                   chanceOfSpreading: Float, canBePlacedOn: List[MinecraftIdentifier]) extends FeatureConfig
...
object GlowLichenFeatureConfig {
    val currentCodec = Codec.record {
        ...
        val canBePlacedOn = Codec[List[MinecraftIdentifier]].fieldOf("can_be_placed_on").forGetter[GlowLichenFeatureConfig](_.canBePlacedOn)
